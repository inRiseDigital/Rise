...
const ScrollStack = ({
  ...
  const setupLenis = useCallback(() => {
    if (useWindowScroll) {
      // Use native scroll with RAF for smooth updates
      let ticking = false;
      ...
      scrollerRef.current = { cleanup: () => window.removeEventListener('scroll', onScroll) };
  ...
  useLayoutEffect(() => {
    ...
    return () => {
      if (useWindowScroll && scrollerRef.current?.cleanup) {
        scrollerRef.current.cleanup();
      }
