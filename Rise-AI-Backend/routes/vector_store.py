...
@vector_store_router.post("/create", summary="Create a new agent with a vector store")
async def create_agent(
    ...
    try:
        ...
        await vector_store.build_vector_store(documents=loaded_content)
        

        return {"message": "Agent created successfully"}
                
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Error processing request: {str(e.args)}")
